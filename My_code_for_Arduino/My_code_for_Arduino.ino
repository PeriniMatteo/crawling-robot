...
servos.  For a more detailed description see video at https://www.youtube.com/watch?v=f2nIKFMyfSg

Intended for use in Arduino UNO
*/
...
float theta1InitialAngle = 50.0; //88?
float theta1Max =  80.0;                          //Can't exceed 110 or else arm will hit bar   //Need theta1Max and theta1Min between 0 and 180 
float theta1Min = 20.0;
...
float delayTime1 = 10.0*deltaTheta1;                                 //The time in ms for the servo to move deltaTheta1

//Servo2 parameters
//This servo moves down with increased angle
const int numTheta2States = 6;
float theta2InitialAngle = 35.0;                
float theta2Max = 60.0;                        //Need theta2Max and theta2Min between 0 and 180 
float theta2Min = 10.0;
...
float delayTime2 = 10.0*deltaTheta2;
...
const int numStates = numTheta1States*numTheta2States; //36
const int numActions = 4;
float Q[numStates][numActions]; //Q[36,4]
...
//Variabili di servizio da controllare
float r = 0.0;
...
int getAction(){
  ...
  if((s1 + 1) != numTheta1States){  //diverso da 6
    allowedActions[0] = 1;
    val = Q[s][0];
    if(val > valMax){
      valMax = val;
...
float getDeltaDistanceRolled(){
  ...
  //distanceNew = float(ultrasonic.ping()); 
    distanceNew = encoderValue;
  
  deltaDistance = distanceNew - distanceOld;
  if (abs(deltaDistance) < 5.0){         //don't count noise
    deltaDistance = 0.0;
  }
...
void loop(){
  ...
  a = getAction();           //Seleziono la Action a tra 0 e 3
  ...
  setPhysicalState(a);    //muovo i servo nella posizione decisa da Action
